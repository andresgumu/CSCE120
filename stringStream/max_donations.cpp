...
int main() {
  ...
  while (std::getline(ifs, line)) {
    ...
    // TODO(student): use the data to determine the winner
    if (donation_count >= 2){ // needs to have at least two donations
      if (max_collector == 0 || current_total > max_total_donation){ // checks current, and then th rest if needed
        max_total_donation = current_total;
